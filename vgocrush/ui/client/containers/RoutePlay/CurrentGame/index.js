...
export default class CurrentGame extends React.Component {
  ...
  componentDidMount() {
    ...
    this._updateTitle = () => {
      ...
      } else if(engine.gameState === 'Over') {
        setTitle('Crashed at ' + numeral(engine.lastCrash).format('0,0.00') + 'x')
      } else if(engine.gameState === 'InProgress') {
  ...
  componentWillUnmount() {
    ...
    engine.events.removeListener('onCrashStart', this._onCrashStart)
    ...
    engine.events.removeListener('onCrashTick', this._onCrashTick)
  ...
  render() {
    ...
    return (
      <div ref="container" className={cn(style.currentGame, disabled && engine.gameState !== 'InProgress' ? style.disabled : null)}>
        { engine.gameState !== 'Over' ? <div className={style.maxProfit}>Max Profit: {numeral(engine.options.maxProfit).format('0,0')}</div> : null }
        <canvas ref="canvas" />

        { disabled && engine.gameState === 'InProgress' ? <div className={style.disabledInProgress}>{CRASH_DISABLED_NEXT_MESSAGE}</div> : null }
      </div>
    )
  }

  _init() {
    if(engine.gameState === 'InProgress') {
      this._showLoaders(true)
    } else {
      this._showLoaders(false)
    }
  ...
  _renderLoader() {
    ...
    if(engine.gameState === 'Over') {
      ctx.lineWidth = 4
      ctx.strokeStyle = !engine.wasBonusRound ? '#fd3173' : '#e4b23c'
      ctx.fillStyle = !engine.wasBonusRound ? 'rgba(253, 49, 115, 0.1)' : 'rgba(228, 178, 60, 0.1)'
      ctx.fill()
    }
  ...
  _renderAxes() {
    ...
    if(engine.gameState === 'Over' && engine.lastCrash) {
      ctx.fillStyle = !engine.wasBonusRound ? '#fd3173' : '#e4b23c'
    }
  ...
  _showLoaders(visible) {
    ...
    //
    // const endColor = (!visible && !!engine.lastCrash) ? (engine.lastCrash < 2 ? '#fd3173' : '#1b85e3') : null
  ...
  _renderGraph() {
    ...
    for(let t = 0, i = 0; t <= this._currentTime; t += 100, i++) {
      let payout = engine.calculateGamePayout(t) - 1
  ...
  _calcGameData() {
    this._currentTime = engine.getElapsedTimeWithLag()
    this._currentGamePayout = engine.calculateGamePayout(this._currentTime)
  }
  ...
  _renderGameData() {
    ...
    if(engine.gameState === 'NotStarted') {
    ...
    if(engine.gameState === 'Over' && !!engine.lastCrash) {
      ctx.font = this._fontSizePx(14) + ' Verdana'
      ctx.fillStyle = !engine.wasBonusRound ? '#fd3173' : '#e4b23c'
      ctx.fillText(parseFloat(engine.lastCrash).toFixed(2) + 'x', this._canvasWidth / 2, this._canvasHeight / 2)
    }

    if(engine.gameState === 'Starting') {
      const timeLeft = (engine.startTime.getTime() / 1000) - (Date.now() / 1000)

      ctx.font = this._fontSizePx(14) + ' Verdana'
      ctx.fillText(timeLeft <= -1.5 ? 'Starting...' : `${Math.max(0, timeLeft).toFixed(1)}s`, this._canvasWidth / 2, this._canvasHeight / 2)
    }
  }
