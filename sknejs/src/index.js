...
function deposit([{ notifyUrl, steamId64, tradeLink, assetIds, meta, discount, depositGroup, includeItems, forceLockItems }], done) {
  ...
  co(function* () {
    ...
    const sellPrice = Math.round(items.reduce((t, i) => t + i.sellPrice, 0) * 100) / 100
...
function onNotification(msg) {
  ...
  if(!action.server || (!config.ignorePriceUpdate && action.method === 'prices.updated')) {
    amqpCh.ack(msg)
    return
  }

  if(!!process.env.NOTIFY_URL) {
    action.server = process.env.NOTIFY_URL
  }
...
function pollPendingOffers() {
  co(function* () {
    ...
    eachSeries(pendingOffers, (offer, done) => {
      co(function* () {
        const itemNames = offer.receipt.itemNames
        const unavailable = itemNames.slice(0)
        const found = []

        const grouped = yield BotItems
          .getAll(r.args(_.uniq(itemNames).map(item => [ item, BOT_ITEM_STATE_AVAILABLE ])), { index: 'nameState' })
          .filter({ bot: offer.providerId || 'unknown' })
          .group('name')
          .coerceTo('array')
          .run(database.connection())
...
co(function* () {
  ...
  // AMQP
  const amqpConnection  = yield amqplib.connect(`amqp://${config.amqp.url}`)
  ...
  app.use((req, res, next) => {
    req.requestIp = req.connection.remoteAddress
    next()
  })
  ...
  app.use((req, res, next) => {
    ...
    ApiKeys
      ...
      .update({
        lastCallAt: r2.now(),
        totalCalls: r2.row('totalCalls').default(0).add(1),
        accessedFrom: r2.row('accessedFrom').default([]).append(req.requestIp).distinct()
      })
      .then(result => {
      ...
      .catch(err => {
        logger.error('Cannot verify api key', key, err)
