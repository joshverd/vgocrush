...
import { Bots, BotItems, VirtualOffers, VirtualOffersGroup, TradeOffers } from '../../lib/documents'
...
function* retryVirtual([{ id, steamId, tradeUrl }]) {
  ...
  if(!!offer.offerId && (offer.state === TRADE_STATE_DECLINED || offer.state === TRADE_STATE_ERROR)) {
    ...
    let { replaced, changes } = yield VirtualOffers
      ...
      .update(r.branch(r.row('state').ne(TRADE_STATE_PENDING), {
        ...
        previousOffers: r.row('previousOffers').default([]).append(r.row('offerId'))
      }, {}), { returnChanges: true })
      .run(connection())
...
function* steamLookup([{ tradeOfferId }]) {
  const tradeOffer = yield TradeOffers.get(tradeOfferId).run(connection())
  if(!tradeOffer) {
    return Promise.reject('Cannot find trade offer')
  }

  const bot = yield Bots.get(tradeOffer.bot).run(connection())
  if(!bot) {
    return Promise.reject('Cannot find bot')
  } else if(!bot.apiKey) {
    return Promise.reject('Bot does not have a valid Steam api key')
  }
  ...
  amqpChannel().publish('skne.control', tradeOffer.bot, new Buffer(JSON.stringify({
    action: 'getOffer',
    control: tradeOffer.offerId
  })), {
    correlationId: id,
    replyTo: 'skne'
  })

  const { success, error, offer } = yield promise
...
function* resendNotification([{ id, state }]) {
  const { replaced, changes } = yield TradeOffers
    .get(id)
    .update({
      state: state,
      manuallyResentAt: r.now()
    }, { returnChanges: true })
    .run(connection())

  if(replaced <= 0) {
    return Promise.reject('Cannot update offer')
  }
